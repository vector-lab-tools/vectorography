...
import numpy as np
from curves import segments, straighten
...
def _names(family: str, style: str, version: str,
    ...
    return {
        ...
        "copyright": ((f"Copyright {author}. " if author else "")
                      ...
                      "Vectorography, copyright 2026 David M. Berry. See "
                      "corpus-manifest.json for the families the space was "
                      "fitted from."),
...
def build_otf(vec: np.ndarray, family: str, style: str = "Regular",
    ...
    for i, ch in enumerate(GLYPHS):
        ...
        for c in dec["contours"][i]:
            if _area(c) <= MIN_AREA:
                continue
            pts = straighten(c, straight) * UPEM if straight > 0 else c * UPEM
            pen.moveTo(tuple(pts[0]))
            # The same construction the navigator renders with, corners and
            # straights included, so what is installed is what was seen.
            for seg in segments(pts):
                if seg[0] == "line":
                    pen.lineTo(tuple(seg[1]))
                else:
                    pen.curveTo(tuple(seg[1]), tuple(seg[2]), tuple(seg[3]))
            pen.closePath()
            drawn = True
        if not drawn:
            pen.moveTo((0, 0)); pen.closePath()
...
def build_ttf(vec: np.ndarray, family: str, style: str = "Regular",
    ...
    for i, ch in enumerate(GLYPHS):
        ...
        if straight > 0:
            contours = np.stack([straighten(c, straight) for c in contours])
