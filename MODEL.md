...
`backend/data/corpus-manifest.json` names the families the space was fitted
from, and travels inside every journey export. It is there for attribution: the
OFL asks that the faces a work derives from can be identified, and a designer
reading an export should be able to see whose drawing went into the fitting.
It is not a component list, and nothing in it is a part of what comes out. A
location in this space is a set of coordinates, and the outline decoded from it
is a linear combination of the whole corpus rather than an assembly of pieces
taken from particular fonts.
